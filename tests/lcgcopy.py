...
class LcgCpSuite(dcachetestcase.SETestCase):
    ...
    def testLcgCp(self):
        ...
        self.assertCommandPass( [self.glite_home + '/bin/lcg-cp', '-b' , '-D', 'srmv2', '-T', 'srmv2', self.localURL, self.remoteURL] )
        self.assertCommandPass( [self.glite_home + '/bin/lcg-cp', '-b' , '-D', 'srmv2', '-T', 'srmv2', self.remoteURL, self.tempURL] )
    ...
    def testLcgGtGsiFtp(self):
        ...
        self.assertCommandPass( [self.glite_home + '/bin/lcg-cp', '-b' , '-D', 'srmv2', '-T', 'srmv2', self.localURL, self.remoteURL] )
        self.assertCommandPass( [self.glite_home + '/bin/lcg-gt', '-b' , '-D', 'srmv2', '-T', 'srmv2', self.remoteURL, 'gsiftp'] )
    ...
    def testLcgLsFile(self):
        ...
        self.assertCommandPass( [self.glite_home + '/bin/lcg-cp', '-b' , '-D', 'srmv2', '-T', 'srmv2', self.localURL, self.remoteURL] )
        self.assertCommandPass( [self.glite_home + '/bin/lcg-ls', '-b' , '-T', 'srmv2', '-l', self.remoteURL] )
